...
# ============================================================
#  SISTEMA DE INTERNAÇÕES — PostgreSQL (Supabase)
#  Sem GitHub sync | Datas ISO (YYYY-MM-DD) | psycopg2 (%s)
# ============================================================
...
# Parser (seu módulo)
#  -> mantenha parser.py com parse_tiss_original(csv_text)
...
def inject_css():
    st.markdown(
        ...
        .kpi-action button{ font-size:.95rem !important; font-weight:700 !important; }

        /* ============================
           PALETA
           ============================ */
        :root{
          --bg-main:#F5F6F7; --bg-card:#FFFFFF; --border:#D0D7DE;
          --text:#24292F; --muted:#6B7280; --primary:#1F6FEB; --primary-hover:#1558B0;
          --radius:8px;
        }
        html,body,.stApp{ background:var(--bg-main)!important; color:var(--text)!important; font-family:"Segoe UI", Roboto, Arial, sans-serif; }

        .app-header{ background:var(--bg-main); padding:10px 12px; margin:-1.2rem -1rem 0.8rem -1rem; border-bottom:1px solid var(--border); }
        ...
        .stTextInput input, .stNumberInput input, .stDateInput input, .stTextArea textarea{
          background:#FFF !important; color:var(--text)!important; border:1px solid var(--border)!important; border-radius:var(--radius)!important; box-shadow:none!important;
        }
...
def create_tables():
    ...
    CREATE TABLE IF NOT EXISTS procedimentos (
      id                            BIGSERIAL PRIMARY KEY,
      internacao_id                 BIGINT REFERENCES internacoes(id) ON DELETE CASCADE,
      ...
      quitacao_valor_amhptiss       NUMERIC(12,2),
      quitacao_guia_complemento     TEXT,
      quitacao_valor_complemento    NUMERIC(12,2),
      quitacao_observacao           TEXT,
      created_at                    TIMESTAMPTZ NOT NULL DEFAULT NOW(),
      updated_at                    TIMESTAMPTZ NOT NULL DEFAULT NOW()
    );

    CREATE INDEX IF NOT EXISTS idx_internacoes_data_internacao ON internacoes (data_internacao);
    CREATE INDEX IF NOT EXISTS idx_internacoes_hospital_data   ON internacoes (hospital, data_internacao);
    CREATE INDEX IF NOT EXISTS idx_proc_internacao             ON procedimentos (internacao_id);
    CREATE INDEX IF NOT EXISTS idx_proc_situacao               ON procedimentos (situacao);
    CREATE INDEX IF NOT EXISTS idx_proc_data                   ON procedimentos (data_procedimento);
    ...
    DROP TRIGGER IF EXISTS trg_upd_internacoes ON internacoes;
    CREATE TRIGGER trg_upd_internacoes
    BEFORE UPDATE ON internacoes
    FOR EACH ROW EXECUTE FUNCTION set_updated_at();

    DROP TRIGGER IF EXISTS trg_upd_procedimentos ON procedimentos;
    CREATE TRIGGER trg_upd_procedimentos
    ...
    try:
        with conn:
            with conn.cursor() as cur:
                for nome in H:
                    cur.execute(
                        "INSERT INTO hospitals (name, active) VALUES (%s, TRUE) ON CONFLICT (name) DO NOTHING",
                        (nome,),
                    )
    finally:
        conn.close()
...
# ============================================================
# UTIL — Datas, Números, Formatação
# ============================================================

def _from_any_to_date(v):
    """Converte v (str dd/mm/yyyy | str yyyy-mm-dd | pd.Timestamp | date) -> date | None"""
    if v is None or v == "":
        return None
...
def _to_iso_date_str(v) -> str | None:
    """Converte v em 'YYYY-MM-DD' para gravar no banco (DATE)."""
    d = _from_any_to_date(v)
    return d.strftime("%Y-%m-%d") if d else None

def _to_ddmmyyyy(v) -> str:
    d = _from_any_to_date(v)
...
# ============================================================
# QUERIES AUXILIARES (pandas + psycopg2)
# ============================================================
...
# ============================================================
# CRUD — Internações / Procedimentos
# ============================================================

def get_hospitais(include_inactive: bool = False) -> list:
    if include_inactive:
        df = query_df("SELECT name FROM hospitals ORDER BY active DESC, name")
    else:
        df = query_df("SELECT name FROM hospitals WHERE active = TRUE ORDER BY name")
    return df["name"].tolist()

def apagar_internacoes(lista_at: list[str]):
    if not lista_at: return
    # Deleta por atendimento
    sql_del_proc = """
        DELETE FROM procedimentos
         WHERE internacao_id IN (
             SELECT id FROM internacoes WHERE atendimento = ANY(%s)
         )"""
    sql_del_int = "DELETE FROM internacoes WHERE atendimento = ANY(%s)"
    exec_sql(sql_del_proc, (lista_at,))
    exec_sql(sql_del_int, (lista_at,))

def atualizar_internacao(internacao_id, paciente=None, convenio=None, data_internacao=None, hospital=None):
    sets, params = [], []
    if paciente is not None:         sets.append("paciente = %s");         params.append(paciente)
    if convenio is not None:         sets.append("convenio = %s");         params.append(convenio)
    if data_internacao is not None:  sets.append("data_internacao = %s");  params.append(_to_iso_date_str(data_internacao))
    if hospital is not None:         sets.append("hospital = %s");         params.append(hospital)
    if not sets: return
    params.append(internacao_id)
    sql = f"UPDATE internacoes SET {', '.join(sets)} WHERE id = %s"
    exec_sql(sql, tuple(params))

def deletar_internacao(internacao_id: int):
    exec_sql("DELETE FROM procedimentos WHERE internacao_id = %s", (internacao_id,))
    exec_sql("DELETE FROM internacoes  WHERE id = %s", (internacao_id,))

def deletar_procedimento(proc_id: int):
    exec_sql("DELETE FROM procedimentos WHERE id = %s", (proc_id,))
...
def criar_internacao(hospital, atendimento, paciente, data, convenio):
    sql = """
        ...
        RETURNING id
    """
    numero_internacao = str(atendimento) if atendimento is not None else None
    params = (numero_internacao, hospital, str(atendimento), paciente, _to_iso_date_str(data), convenio)
    rows = exec_sql(sql, params, fetch=True)
    return int(rows[0]["id"])

def criar_procedimento(internacao_id, data_proc, profissional, procedimento,
                       situacao="Pendente", observacao=None, is_manual=False,
    ...
    sql = """
        ...
        ON CONFLICT (internacao_id, data_procedimento)
          WHERE is_manual = FALSE
        DO NOTHING
    """
    params = (
        internacao_id,
        _to_iso_date_str(data_proc),
        profissional, procedimento, situacao, observacao,
        bool(is_manual), aviso, grau_participacao
    )
    exec_sql(sql, params)
...
def existe_procedimento_no_dia(internacao_id, data_proc):
    sql = """
        ...
        WHERE internacao_id = %s
          AND data_procedimento = %s
          AND is_manual = FALSE
        LIMIT 1
    """
    rows = exec_sql(sql, (internacao_id, _to_iso_date_str(data_proc)), fetch=True)
...
def atualizar_procedimento(proc_id, procedimento=None, situacao=None,
    ...
    sets, params = [], []
    if procedimento is not None:      sets.append("procedimento = %s");      params.append(procedimento)
    if situacao is not None:          sets.append("situacao = %s");          params.append(situacao)
    if observacao is not None:        sets.append("observacao = %s");        params.append(observacao)
    if grau_participacao is not None: sets.append("grau_participacao = %s"); params.append(grau_participacao)
    if aviso is not None:             sets.append("aviso = %s");             params.append(aviso)
    if not sets: return
    params.append(proc_id)
    sql = f"UPDATE procedimentos SET {', '.join(sets)} WHERE id = %s"
    exec_sql(sql, tuple(params))
...
def quitar_procedimento(proc_id, data_quitacao=None, guia_amhptiss=None, valor_amhptiss=None,
    ...
    sql = """
        UPDATE procedimentos
           SET quitacao_data = %s,
               quitacao_guia_amhptiss = %s,
               quitacao_valor_amhptiss = %s,
               quitacao_guia_complemento = %s,
               quitacao_valor_complemento = %s,
               quitacao_observacao = %s,
               situacao = 'Finalizado'
         WHERE id = %s
    """
    params = (
        _to_iso_date_str(data_quitacao),
        guia_amhptiss, _to_float_or_none(valor_amhptiss),
        guia_complemento, _to_float_or_none(valor_complemento),
        quitacao_observacao, proc_id
    )
    exec_sql(sql, params)
...
def reverter_quitacao(proc_id: int):
    sql = """
        UPDATE procedimentos
         ...
         WHERE id = %s
    """
    exec_sql(sql, (proc_id,))

def get_internacao_by_atendimento(att):
    return query_df("SELECT * FROM internacoes WHERE atendimento = %s", (str(att),))

def get_procedimentos(internacao_id):
    return query_df("SELECT * FROM procedimentos WHERE internacao_id = %s", (internacao_id,))
...
app_header("Sistema de Internações — PostgreSQL (Supabase)",
           "Importação, edição, quitação e relatórios com persistência em Postgres")
...
with tabs[0]:
    ...
    with colf1:
        filtro_hosp_home = st.selectbox(
            "Hospital", ["Todos"] + get_hospitais(), index=0, key="home_f_hosp"
        )
    with colf2:
        st.write(" ")
    ...
    # Base: todos os procedimentos (join internacoes)
    sql_all = """
        SELECT
    ...
    def _safe_pt_or_ts_to_date(s):
        if isinstance(s, pd.Timestamp): return s.date()
        return _from_any_to_date(s)

    # Filtros
    if df_all.empty:
        df_f = df_all.copy()
    else:
        df_all["_int_dt"]  = df_all["data_internacao"].apply(_safe_pt_or_ts_to_date)
        df_all["_proc_dt"] = df_all["data_procedimento"].apply(_safe_pt_or_ts_to_date)
    ...
    with c1:
        kpi_row([{"label":"Pendentes", "value": f"{tot_pendente}", "hint": "Todos os procedimentos (geral/filtrado)"}], extra_class="center")
    ...
    with c2:
        kpi_row([{"label":"Finalizadas", "value": f"{tot_finalizado}", "hint": "Todos os procedimentos (geral/filtrado)"}], extra_class="center")
    ...
    with c3:
        kpi_row([{"label":"Não Cobrar", "value": f"{tot_nao_cobrar}", "hint": "Todos os procedimentos (geral/filtrado)"}], extra_class="center")
    ...
    # Listagem por status (ordenada por data da internação — mais antigas primeiro)
        ...
        if df_f.empty:
            ...
            if df_status.empty:
                ...
                def _safe_pt_date_int(s):
                    return _safe_pt_or_ts_to_date(s)
                ...
                for _, r in df_ints.iterrows():
                    ...
                    with i3:
                        st.markdown(f"**Data internação:** {_to_ddmmyyyy(r.get('data_internacao'))}")
                    with i4:
                        ...
                        if st.button("🔎 Abrir na Consulta", key=btn_key, use_container_width=True):
                            ...
                            # Disparo imediato do switch via JS
                            js = """
                            <script>
                            (function(){
                              ...
                              const tick = setInterval(() => {
                                ...
                                for (const t of tabs) {
                                  const txt = norm(t.textContent || t.innerText);
                                  if (txt.includes(norm(target))) { t.click(); clearInterval(tick); return; }
                                }
...
with tabs[1]:
    ...
    with col_btn:
        if st.button("Criar internação", key="imp_btn_criar_int", type="primary"):
            if not att_new:
                ...
                nid = criar_internacao(hosp_new, att_new, pac_new, data_new, conv_new)
    ...
    if arquivo:
        ...
        st.markdown(
            f"<div>🔎 {len(pares)} par(es) (atendimento, data) após filtros. Regra: {pill('1 auto por internação/dia')} (manuais podem ser vários).</div>",
            unsafe_allow_html=True
        )
            ...
            if st.button("Gravar no banco", type="primary"):
                ...
                for (att, data_proc) in pares:
                    ...
                    criar_procedimento(
                        ...
                        observacao=None, is_manual=False, aviso=aviso_dia or None, grau_participacao=None
                    )
                    total_criados += 1
...
# ============================================================
# 🔍 2) CONSULTAR (edição, exclusões, ver quitação)
# ============================================================
with tabs[2]:
    ...
    if codigo:
        ...
        if df_int.empty:
            ...
            df_int = df_int.copy()
            df_int["data_internacao"] = df_int["data_internacao"].apply(_to_ddmmyyyy)
            ...
            with st.container():
                ...
                with c3:
                    ...
                    dt_atual = _from_any_to_date(data_atual) or date.today()
                    nova_data = st.date_input("Data da internação:", value=dt_atual)
                with c4:
                    todos_hospitais = get_hospitais(include_inactive=True)
                    idx_h = todos_hospitais.index(df_int["hospital"].iloc[0]) if (df_int["hospital"].iloc[0] in todos_hospitais) else 0
                    novo_hospital = st.selectbox("Hospital:", todos_hospitais, index=idx_h)
            ...
            # Garantias e formatação
            if df_proc.empty:
                df_proc = pd.DataFrame(columns=["id","data_procedimento","profissional","procedimento","situacao","observacao","aviso","grau_participacao"])
            df_proc["procedimento"]      = df_proc.get("procedimento", pd.Series(dtype=str)).fillna("Cirurgia / Procedimento")
            ...
            # Ordenação por data do procedimento (mais antigas primeiro)
            df_proc["_data_dt"] = df_proc["data_procedimento"].apply(_from_any_to_date)
            ...
            with col_add:
                # Data da internação (recalcula in loco da fonte)
                df_int_raw = get_internacao_by_atendimento(codigo)
                data_internacao_str = df_int_raw["data_internacao"].iloc[0] if not df_int_raw.empty else None
                dt_internacao = _from_any_to_date(data_internacao_str) or date.today()
                    ...
                    if data_proc < dt_internacao:
                        ...
                        if profissional == "(selecione)":
                            ...
                            criar_procedimento(
                                ...
                                situacao=situacao, observacao=(observacao or None),
                                is_manual=True, aviso=None,
            ...
            # Precisamos recarregar df_proc em estado mais bruto para checar status:
            df_proc_re = query_df(
                """
                SELECT id, data_procedimento, profissional, procedimento, situacao, observacao, aviso, grau_participacao
                FROM procedimentos
                ...
                if "show_quit_id" in st.session_state and st.session_state["show_quit_id"]:
                    ...
                    if not df_q.empty:
                        ...
                        with c2:
                            ...
                            st.markdown(f"**Data procedimento:** {_to_ddmmyyyy(q['data_procedimento']) or '-'}")
                        ...
                        with c4:
                            st.markdown(f"**Data da quitação:** {_to_ddmmyyyy(q['quitacao_data']) or '-'}")
...
if REPORTLAB_OK:
    def _pdf_cirurgias_por_status(df, filtros):
        ...
        filtros_txt = (f"Período: {filtros['ini']} a {filtros['fim']}  |  "
                       f"Hospital: {filtros['hospital']}  |  Status: {filtros['status']}")
        ...
        header_labels = ["Atendimento", "Aviso", "Convênio", "Paciente",
...
if REPORTLAB_OK:
    def _pdf_quitacoes(df, filtros):
        ...
        header = ["Convênio", "Paciente", "Profissional", "Data", "Atendimento",
        ...
        numeric_cols = [7, 8]; center_cols = [3, 4, 9]
        ...
        for c in numeric_cols: style_cmds.append(("ALIGN", (c,1), (c,-1), "RIGHT"))
        for c in center_cols:  style_cmds.append(("ALIGN", (c,1), (c,-1), "CENTER"))
...
with tabs[3]:
    ...
    if not df_rel.empty:
        df_rel["_data_dt"] = df_rel["data_procedimento"].apply(_from_any_to_date)
    ...
    with colb1:
        if st.button("Gerar PDF", type="primary"):
            if df_rel.empty:
                ...
                filtros = {"ini": dt_ini.strftime("%d/%m/%Y"), "fim": dt_fim.strftime("%d/%m/%Y"), "hospital": hosp_sel, "status": status_sel}
    ...
    with colq2:
        hoje = date.today(); ini_default_q = hoje.replace(day=1)
    ...
    if not df_quit.empty:
        df_quit["_quit_dt"] = df_quit["quitacao_data"].apply(_from_any_to_date)
        mask_q = (df_quit["_quit_dt"].notna()) & (df_quit["_quit_dt"] >= dt_ini_q) & (df_quit["_quit_dt"] <= dt_fim_q)
        df_quit = df_quit[mask_q].copy()
        if hosp_sel_q != "Todos": df_quit = df_quit[df_quit["hospital"] == hosp_sel_q]

        df_quit = df_quit.sort_values(by=["_quit_dt", "convenio", "paciente"])
        df_quit["data_procedimento"] = df_quit["data_procedimento"].apply(lambda s: _to_ddmmyyyy(s))
        ...
        if st.button("Gerar PDF (Quitações)", type="primary"):
            if df_quit.empty:
                ...
                if not REPORTLAB_OK:
                    ...
                    filtros_q = {"ini": dt_ini_q.strftime("%d/%m/%Y"), "fim": dt_fim_q.strftime("%d/%m/%Y"), "hospital": hosp_sel_q}
...
with tabs[4]:
    ...
    if hosp_sel == "Todos":
        ...
        df_quit = query_df(sql)
    else:
        sql = base + " AND I.hospital = %s ORDER BY P.data_procedimento, I.hospital, I.atendimento"
        df_quit = query_df(sql, (hosp_sel,))

    if df_quit.empty:
        st.info("Não há cirurgias com status 'Enviado para pagamento' para quitação.")
    else:
        # Converte campos numéricos e datas para edição
        df_quit["quitacao_valor_amhptiss"] = pd.to_numeric(df_quit["quitacao_valor_amhptiss"], errors="coerce")
        df_quit["quitacao_valor_complemento"] = pd.to_numeric(df_quit["quitacao_valor_complemento"], errors="coerce")
        df_quit["data_procedimento"] = df_quit["data_procedimento"].apply(_to_ddmmyyyy)
        df_quit["quitacao_data"] = df_quit["quitacao_data"].apply(_from_any_to_date)
        ...
        with col_quit:
            if st.button("💾 Gravar quitação(ões)", type="primary"):
                ...
                for _, row in compare.iterrows():
                    ...
                    data_q = _from_any_to_date(row["quitacao_data_new"])
                    if not data_q:
...
with tabs[5]:
    ...
    st.caption("🔗 Persistência **PostgreSQL (Supabase)** ativa — conexão via `st.secrets['SUPABASE_DB_URI']`.")
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    if st.button("Carregar procedimentos", key="btn_carregar_proc", type="primary"):
        ...
        if chosen == "Todos":
            ...
            sql = base + " WHERE I.hospital = %s ORDER BY P.data_procedimento DESC, P.id DESC"
            df = query_df(sql, (chosen,))
        # Formata data_procedimento para exibição:
        if not df.empty:
            df["data_procedimento"] = df["data_procedimento"].apply(_to_ddmmyyyy)
    ...
    if chosen_prof == "Todos":
        ...
        df_prof = query_df(sql)
    else:
        sql = base_prof + " AND I.hospital = %s GROUP BY profissional ORDER BY total DESC"
        df_prof = query_df(sql, (chosen_prof,))
    ...
    if chosen_conv == "Todos":
        ...
        df_conv = query_df(sql)
    else:
        sql = base_conv + " AND I.hospital = %s GROUP BY I.convenio ORDER BY total DESC"
        df_conv = query_df(sql, (chosen_conv,))
